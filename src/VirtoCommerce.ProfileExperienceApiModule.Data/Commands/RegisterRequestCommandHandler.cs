...
#pragma warning restore S107
        ...
        public virtual async Task<RegisterOrganizationResult> Handle(RegisterRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            var IsPhoneNumber = Regex.Match(request.Contact.PhoneNumber, @"^(\+[0-9]{12})$").Success;

            if (!IsPhoneNumber)
            {
                result = await HandleErrorAsync(request.Account, request.LanguageCode, request.Contact, ModuleConstants.RegistrationError.PhoneNumber);
                return result;
            }
            if (!request.Account.Email.Contains("@") || string.IsNullOrEmpty(request.Account.Email))
            {
                result = await HandleErrorAsync(request.Account, request.LanguageCode, request.Contact, ModuleConstants.RegistrationError.Email);
                return result;
            }
        ...
        private async Task<RegisterOrganizationResult> HandleErrorAsync(Account account, string LanguageCode, RegisteredContact contact, string Error)
        {
            var RequiredEmailVerification = EmailVerificationFlow == ModuleConstants.RegistrationFlows.EmailVerificationRequired;
            var result = new RegisterOrganizationResult();
            result.AccountCreationResult = new AccountCreationResult();
            result.Contact = await ToContact(contact, ToApplicationUser(account), LanguageCode, RequiredEmailVerification);
            result.AccountCreationResult.Succeeded = false;
            result.AccountCreationResult.AccountName = account.UserName;
            result.AccountCreationResult.Email = account.Email;
            result.AccountCreationResult.RequireEmailVerification = RequiredEmailVerification;
            result.AccountCreationResult.Errors = new List<RegistrationError> { new RegistrationError()
                { Code = "RegistrationError" , Description = Error , Parameter = null } };
            //SetErrorResult(result, "RegistrationError", "Phone Number is not valid", new CancellationTokenSource());
            return result;
        }
        ...
        protected virtual async Task ProcessRequestAsync(RegisterRequestCommand request, RegisterOrganizationResult result, CancellationTokenSource tokenSource)
        {
            ...
            account.PhoneNumber = request.Contact.PhoneNumber;
            request.Account.PhoneNumber = request.Contact.PhoneNumber;
            ...
            catch (Exception e)
            {
                ...
                {
                    SetErrorResult(result, "SQl Exception", "Phone Number can not duplicate", tokenSource);
                }
                else
                {
                    SetErrorResult(result, e.GetType().Name, e.Message, tokenSource);
                }
            }
        ...
        protected virtual async Task<Contact> ToContact(RegisteredContact contact, ApplicationUser account, string language, bool requireEmailVerification)
        {
            ...
            if (result.DynamicProperties != null)
            {
                if (result.DynamicProperties.Count() > 0)
                {
                    var property = result.DynamicProperties.FirstOrDefault(x => x.Name == "Channel");
                    if (property != null)
                    {
                        result.DynamicProperties.FirstOrDefault(x => x.Name == "Channel").ValueType = Platform.Core.DynamicProperties.DynamicPropertyValueType.ShortText;
                    }
                }
            }
